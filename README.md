# FoodByte Frontend Service

This repository contains the application UI code for the **Frontend Service** microfrontend, which serves as the user-facing component of the broader FoodByte platform.
...
- `github.com/yourname/foodbyte-user-service`       ← application code
...
- `github.com/yourname/foodbyte-frontend`            ← application code (**this repo**)
...
This frontend does **not** push deployments to the cluster directly. Deployments are handled asynchronously, powered by Flux CD viewing the `foodbyte-helm-charts` configuration.

The fully automated delivery flow operates as follows:

1. You push new code to the `foodbyte-frontend` repository.
2. GitHub Actions CI automatically triggers — it lints, tests, builds the UI assets (Vite/React/etc.), builds the new Docker image, and pushes it to your GHCR registry with the image tag set as the `git commit SHA`.
3. The CI pipeline then opens a PR (or auto-commits) on `foodbyte-helm-charts`, updating the specific `image.tag` contained in the `values.yaml` for the Frontend Service.
