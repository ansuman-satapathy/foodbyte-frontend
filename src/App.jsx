...
export default function App() {
  ...
  const [activeMerchantOrders, setActiveMerchantOrders] = useState(0);
    ...
    <div className="min-h-screen bg-[#FDFCFB] flex flex-col font-sans text-gray-900 overflow-x-hidden relative">
      ...
      <main className="flex-1">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {currentView === 'checkout' && <CheckoutView apiFetch={apiFetch} cart={cart} setCart={setCart} onOrderPlaced={() => setCurrentView('orders')} showAlert={showAlert} showConfirm={showConfirm} />}
...
const CheckoutView = ({ apiFetch, cart, setCart, onOrderPlaced, showAlert, showConfirm }) => {
  ...
  if (isCartEmpty) return (<div className="max-w-xl mx-auto py-20 text-center space-y-8 animate-in fade-in zoom-in duration-500"><div className="w-32 h-32 bg-gray-50 rounded-full flex items-center justify-center mx-auto text-6xl grayscale opacity-30 shadow-inner">🚲</div><div><h2 className="text-4xl font-black text-gray-900 tracking-tight mb-2">Empty Tray.</h2><p className="text-gray-400 font-medium leading-relaxed">Delicious things await.</p></div><button onClick={() => setCurrentView('home')} className="bg-gray-900 text-white px-10 py-5 rounded-full text-lg font-black hover:bg-black transition-all transform hover:scale-105 shadow-2xl">Start Browsing</button></div>);
  return (
